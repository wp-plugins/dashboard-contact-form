...
function cdw_widget_options_form() {
	...
	if(isset($_POST['action'])) {
	 ...
	 $cdw_data = array( 
						...
						'placeholder_title' 	=>  esc_textarea(wp_kses_post($_POST['placeholder_title']) ),
						'placeholder_message' 	=>  wp_kses_post( $_POST['form_message'] ),
...
//if(isset($_POST) && WP_DEBUG && WP_DEBUG_DISPLAY) echo "POST: <pre>".print_r($_POST,true)."</pre>";
	...
	<?php 
	// is cf enabled? 
	//$cf_enabled = $cdw_data['use_form'] != "on" ? "readonly=\"readonly\" " :  ""; ?>
	...
	<tr><td><label for="form_title"><?php _e('Form title:','cdw_widget'); ?></label></td><td><input type="text" class="style-input" name="form_title" value="<?php echo(isset($cdw_data['form_title']) ? $cdw_data['form_title'] : __('Contact us:','cdw_widget')); ?>" /></td></tr>
	
	<tr><td><label for="placeholder_title"><?php _e('Title placeholder:','cdw_widget'); ?></label></td><td><input type="text" class="style-input" name="placeholder_title" value="<?php echo(isset($cdw_data['placeholder_title']) ? $cdw_data['placeholder_title'] : __('Message Title','cdw_widget')); ?>" /></td></tr>
	<tr><td><label for="form_message"><?php _e('Message placeholder:','cdw_widget'); ?></label></td><td><input type="text" class="style-input" name="form_message" value="<?php echo(isset($cdw_data['form_message']) ? $cdw_data['placeholder_message'] : __('Type here your message.','cdw_widget') ); ?>" /></td></tr>
	...
	<table><tbody>
	<tr><td><?php _e('Display for roles:','cdw_widget'); ?></td><td><?php _e('Enable contact form for roles:','cdw_widget'); ?></td></tr> <?php  echo "\n";
	...
	echo "<td><input type=\"checkbox\" name=\"cdw_role[]\" class=\"cdw_role\" ".$readonly."id=\"role_".$rolename."\" value=\"".$rolename."\" ".$checked."><label class=\"cdw_role_label\" for=\"role_".$rolename."\">".translate_user_role($rolename)."</label></td>\n";
	...
	echo "<td><input type=\"checkbox\" name=\"cdw_role_cf[]\" class=\"cdw_role\" ".$readonly_cf."id=\"role_".$rolename."_cf\" value=\"".$rolename."\" ".$checked_cf."><label class=\"cdw_role_label\" for=\"role_".$rolename."_cf\">".translate_user_role($rolename)."</label></td></tr>\n";
	}
	?></tbody></table>
	<input class="button-primary" type="submit" value="<?php _e('Save','cdw_widget'); ?>" />
	</form>
	<p><sup>*</sup><?php _e('E-mail address where will be delivered all messages from the contact form','cdw_widget'); ?></p>
	<br /><hr />
