...
function cdw_widget_options(){
	...
	if(@$_GET['action'] == 'forgotten_password' )
	{
		...
		{
			echo '<div id="message" class="error"><p>'.sprintf(__('Failed to send message to site admin on <strong>%s</strong>.','cdw_widget'),get_option('site_admin')).'</p></div>';
		}
	}
	...
	{
		if (isset($_POST['cdw_pass'])) $_SESSION['cdw_verified'] = md5($_POST['cdw_pass']);
		if(!isset($_GET['action'])) cdw_widget_options_form();
	}
	else
	{
	if(@$_GET['action'] !== 'reset_password')
		...
		<input class="button-primary" type="submit" value="<?php _e('Enter','cdw_widget'); ?>" />&nbsp;&nbsp;<a class="button button-primary button-large" href="<?php echo admin_url('options-general.php?page=cdw_widget_options')."&amp;action=forgotten_password"; ?>" onclick="return confirm('<?php _e("Do you really want to reset password?","cdw_widget");?>');"><?php _e('Reset password (site admin only)','cdw_widget'); ?></a>
		</form>
...
//if(isset($_POST) && WP_DEBUG && WP_DEBUG_DISPLAY) echo "POST: <pre>".print_r($_POST,true)."</pre>";
	...
	*/?>
	...
	<table><tbody>
	<?php $has_empty = $cdw_pass == md5('') ? true : false; ?>
...
function cdw_reset_mail(){
	...
	$subject = sprintf(__('Resetting Dashboard Contact Form password for %s','cdw_widget'),get_bloginfo('url'));
